...
func (s *Storage) getItemsEndpoint(w http.ResponseWriter, r *http.Request) {
	...
	items, err := s.GetItems(gameVersion, language)
	if err != nil {
		s.log.Errorf("Could not get Items for gameversion %s, language %s: %s", gameVersion, language, err)
